...
export const Logo: React.FC<LogoProps> = ({ src, alt }) => (
    <Link href="/" className="text-2xl font-bold text-white">
        <Image src={src} alt={alt} className="h-14 w-auto" width={300} height={150} />
    </Link>
);
    ...
    return (
        <div className="relative group">
            {hasSubItems ? (
                ...
                <div className="relative text-center">
                    <Link href={href || '#'} className="text-xs text-white uppercase tracking-widest pb-1">
                        {label}
                    </Link>
                    <span className="block h-[1px] bg-white absolute left-0 bottom-0 w-full transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-in-out origin-left"></span>
                </div>
...
export const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, setIsOpen, children }) => {
    return (
        <>
            {/* Overlay */}
            <div
                className={`fixed top-16 left-0 right-0 bottom-0 bg-black bg-opacity-50 z-40 transition-opacity duration-300 ease-in-out ${isOpen ? 'opacity-100 xl:hidden' : 'opacity-0 pointer-events-none'
                    }`}
            ...
            {/* Mobile Menu */}
            <div
                className={`fixed top-16 right-0 w-3/4 sm:w-1/2 md:w-1/3 lg:w-1/4 h-[calc(100vh_-_4rem)] bg-gold z-50 transform transition-transform duration-300 ease-in-out xl:hidden ${isOpen ? 'translate-x-0' : 'translate-x-full'
                    }`}
                id="mobile-menu"
            >
...
export const Menu: React.FC<MenuProps> = ({ children }) => {
    ...
    return (
        <nav className="bg-gold shadow-md fixed w-full z-50">
